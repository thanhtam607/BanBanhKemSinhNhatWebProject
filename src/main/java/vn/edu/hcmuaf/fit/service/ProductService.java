...
public class ProductService {
    ...
    public static List<Comment> comt(){
        ...
        if(statement != null)
            try{
                ResultSet rsCmt = stmt1.executeQuery("SELECT MaSP, KHACHHANG.TENKH,BinhLuan,NgayBL from Comments, KHACHHANG where KHACHHANG.MAKH = Comments.MAKH");
                while(rsCmt.next()){
                            list.add(new Comment(rsCmt.getString(1), rsCmt.getString(2), rsCmt.getString(3), rsCmt.getString(4)));
                        }
            }
            catch (SQLException e){
                throw new RuntimeException(e);
...
//        System.out.println(ProductService.comt().toString());
    }
